...
system: `Du bist ein präzises Analyse-Tool. Extrahiere die wichtigsten Fakten und Zusammenhänge.
...
Kein Fülltext.`, placeholder: "Text, Artikel, Recherche-Ergebnis hier einfügen…", }, { id: "entscheidung", label: "Entscheidung", icon: "⚖️", system: `Du bist ein nüchternes Entscheidungs-Tool.
Antworte in diesem Format:
...
Direkt. Kein Coaching.`, placeholder: "Welche Entscheidung steht an? Kontext beschreiben…", }, { id: "impuls", label: "Impuls", icon: "⚡", system: `Du bist ein Sofort-Interventions-Tool für akute Drang- oder Impulssituationen.
Antworte in diesem Format:
...
Kein Coaching. Keine Motivation. Nur Orientierung.`,
...
function mdHtml(t) {
return t
.replace(/^## (.+)$/gm, ‘<h3 style="font-size:15px;font-weight:800;margin:14px 0 8px">$1</h3>’)
.replace(/**(.+?)**/g, “<strong>$1</strong>”)
.replace(/^- (.+)$/gm, ‘<div style="padding:3px 0 3px 12px;border-left:2px solid #e8e4de;margin:3px 0">$1</div>’)
...
return <div style={{ background: S.card, borderRadius: 14, padding: “16px 18px”, marginBottom: 10, …style }}>{children}</div>;
}

function Spinner() {
return <span style={{ display:“inline-block”, width:16, height:16, border:“2.5px solid #ddd”, borderTopColor:S.text, borderRadius:“50%”, animation:“spin 0.7s linear infinite” }} />;
}
...
return (
<div onClick={onToggle} style={{ display:“flex”, alignItems:“center”, gap:14, padding:“13px 0”, borderBottom:`1px solid ${S.border}`, cursor:“pointer” }}>
<div style={{ width:24, height:24, borderRadius:“50%”, flexShrink:0, border:`2px solid ${checked ? accent : S.border}`, background:checked ? accent : “transparent”, display:“flex”, alignItems:“center”, justifyContent:“center”, transition:“all 0.2s” }}>
{checked && <svg width="11" height="9" viewBox="0 0 13 10" fill="none"><path d="M1 5L5 9L12 1" stroke="white" strokeWidth="2" strokeLinecap="round"/></svg>}
</div>
<div style={{ flex:1 }}>
<div style={{ fontSize:14, fontWeight:700, textDecoration:checked?“line-through”:“none”, color:checked?S.muted:S.text }}>{item.name}</div>
...
return <div style={{ background:”#fce4ec”, color:”#b71c1c”, borderRadius:10, padding:“12px 14px”, fontSize:13, marginBottom:10 }}>{msg}</div>;
}

function Btn({ onClick, disabled, children, outline }) {
return (
<button onClick={onClick} disabled={disabled} style={{
width:“100%”, padding:“15px”, border: outline ? `2px solid ${S.text}` : “none”,
background: outline ? “transparent” : (disabled ? “#ccc” : S.text),
color: outline ? S.text : (disabled ? “#999” : S.bg),
borderRadius:14, fontSize:15, fontWeight:800, fontFamily:“inherit”,
...
// ── Central Claude call — FIXED model string ──
...
setCiError(e instanceof Error ? e.message : “Unbekannter Fehler”);
setCiStep(“form”);
}
}
...
return (
<div style={{ minHeight:“100vh”, background:S.bg, fontFamily:”-apple-system,‘SF Pro Text’,system-ui,sans-serif”, color:S.text, maxWidth:430, margin:“0 auto”, position:“relative”, display:“flex”, flexDirection:“column” }}>
<style>{`*{box-sizing:border-box;-webkit-tap-highlight-color:transparent} input[type=range]{-webkit-appearance:none;height:4px;border-radius:4px;background:#e8e4de} input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:22px;height:22px;border-radius:50%;background:#1a1a1a;cursor:pointer} textarea{resize:none;outline:none;border:none;font-family:inherit} @keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}} @keyframes spin{to{transform:rotate(360deg)}}`}</style>

```
  <div style={{ flex:1, overflowY:"auto", padding:"28px 16px 96px" }}>
    {/* Header */}
    <div style={{ marginBottom:22, animation:"fadeUp 0.4s ease both" }}>
      <div style={{ fontSize:10, fontFamily:"monospace", color:S.muted, letterSpacing:"0.08em", marginBottom:4, textTransform:"uppercase" }}>{today}</div>
      <h1 style={{ fontSize:26, fontWeight:800, letterSpacing:"-0.03em", lineHeight:1.1, margin:0 }}>
        {{ supps:"Supplements", checkin:"Check-in", verlauf:"Verlauf", denken:"Denken" }[tab]}
      </h1>
    </div>
      ...
      <div style={{ animation:"fadeUp 0.35s ease both" }}>
        <Card style={{ marginBottom:16 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
            <span style={{ fontSize:11, fontWeight:600, color:S.muted, letterSpacing:"0.06em", textTransform:"uppercase" }}>Fortschritt</span>
          ...
          <div style={{ height:5, background:S.border, borderRadius:10, overflow:"hidden" }}>
            <div style={{ height:"100%", borderRadius:10, background:`linear-gradient(90deg,${S.morning},${S.evening})`, width:`${(doneSupps/TOTAL_SUPPS)*100}%`, transition:"width 0.4s ease" }} />
          </div>
        </Card>
        ...
        {[
          ...
          <div key={sec.label} style={{ marginBottom:14 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, paddingBottom:8, marginBottom:2 }}>
            ...
            <div style={{ borderRadius:14, overflow:"hidden", background:S.card, padding:"0 16px" }}>
        ...
        {doneSupps === TOTAL_SUPPS && (
          <Card style={{ textAlign:"center", background:S.text, color:S.bg, marginBottom:14 }}>
        ...
        <button onClick={() => { setSuppChecked({}); ls.set("supp_"+DAY_KEY(), "{}"); }}
          style={{ width:"100%", padding:"13px", background:"transparent", border:`1.5px solid ${S.border}`, borderRadius:14, fontSize:14, fontWeight:700, cursor:"pointer", color:S.muted }}>
          ↺ Zurücksetzen
        </button>
      </div>
    )}
      ...
      <div style={{ animation:"fadeUp 0.35s ease both" }}>
        {ciStep === "form" && <>
          ...
          <Card style={{ marginBottom:10 }}>
            ...
            <textarea rows={3} placeholder="z.B. Stress, Langeweile, Schlafmangel, Streit…"
              value={ci.trigger} onChange={e => setCi(c => ({...c, trigger:e.target.value}))}
              style={{ width:"100%", background:"#f7f5f2", borderRadius:8, padding:"10px 12px", fontSize:13, lineHeight:1.5 }} />
          </Card>
          ...
          <Btn onClick={submitCheckin} disabled={!ci.trigger.trim()}>Auswerten →</Btn>
        </>}

        {ciStep === "loading" && (
          <div style={{ textAlign:"center", padding:"70px 0" }}>
            <div style={{ width:34, height:34, border:"3px solid #ddd", borderTopColor:S.text, borderRadius:"50%", animation:"spin 0.7s linear infinite", margin:"0 auto 16px" }} />
            <p style={{ color:S.muted, fontSize:13 }}>Analyse läuft…</p>
          </div>
        )}

        {ciStep === "result" && <>
          {ciRisk && (
            <div style={{ background:RISK_META[ciRisk].bg, borderRadius:14, padding:"12px 16px", display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              <div style={{ width:10, height:10, borderRadius:"50%", background:RISK_META[ciRisk].dot, flexShrink:0 }} />
              <span style={{ fontSize:13, fontWeight:700, color:RISK_META[ciRisk].text, textTransform:"capitalize" }}>Risiko: {ciRisk}</span>
            </div>
          )}
    ...
    {tab === "verlauf" && (
      <div style={{ animation:"fadeUp 0.35s ease both" }}>
        {checkins.length === 0 ? (
          <Card style={{ textAlign:"center", padding:"40px 20px", color:S.muted }}>
            <div style={{ fontSize:32, marginBottom:10 }}>📊</div>
            <div style={{ fontWeight:700, marginBottom:6 }}>Noch keine Daten</div>
            <div style={{ fontSize:13 }}>Mach deinen ersten Check-in.</div>
          </Card>
        ) : <>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:14 }}>
            {[{label:"Ø Stimmung",value:avgMood},{label:"Ø Drang",value:avgUrge},{label:"Hohes Risiko",value:highRisk}].map(s => (
              <Card key={s.label} style={{ textAlign:"center", padding:"12px 8px", marginBottom:0 }}>
                ...
                <div style={{ fontSize:10, color:S.muted, marginTop:3, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>{s.label}</div>
              </Card>
            ))}
          </div>

          <Card style={{ marginBottom:14, paddingLeft:8, paddingRight:8 }}>
            <div style={{ fontSize:11, fontWeight:700, color:S.muted, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:10 }}>
            ...
            <ResponsiveContainer width="100%" height={190}>
              <LineChart data={chartData} margin={{top:4,right:12,left:-20,bottom:4}}>
                ...
                <Line type="monotone" dataKey="Stimmung" stroke={S.morning} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="Drang" stroke={S.evening} strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </Card>
          ...
          <Card style={{ marginBottom:14 }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
              <div>
                <div style={{ fontSize:13, fontWeight:800 }}>Muster-Analyse</div>
                <div style={{ fontSize:11, color:S.muted }}>KI · letzte {Math.min(checkins.length,14)} Einträge</div>
              </div>
              <button onClick={runPattern} disabled={patternLoading || checkins.length < 2}
                style={{ background:S.text, color:S.bg, border:"none", borderRadius:10, padding:"8px 14px", fontSize:12, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:6 }}>
                {patternLoading ? <Spinner /> : "Analysieren"}
              </button>
            </div>
            {patternError && <ErrorBox msg={patternError} />}
            {checkins.length < 2 && <div style={{ fontSize:12, color:S.muted, fontStyle:"italic" }}>Mind. 2 Check-ins nötig.</div>}
            {patternResult && (
              <div style={{ fontSize:13, lineHeight:1.7, borderTop:`1px solid ${S.border}`, paddingTop:12 }}
                dangerouslySetInnerHTML={{ __html:mdHtml(patternResult) }} />
            )}
          </Card>

          <div style={{ fontSize:11, fontWeight:700, color:S.muted, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:8 }}>Letzte Einträge</div>
          {[...checkins].reverse().slice(0,7).map((c,i) => {
            const rm = c.risk ? RISK_META[c.risk] : null;
            return (
              <Card key={i} style={{ marginBottom:8, display:"flex", alignItems:"center", gap:12, padding:"12px 14px" }}>
                <div style={{ fontFamily:"monospace", fontSize:11, color:S.muted, flexShrink:0 }}>{fmtDate(c.date)}</div>
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:S.muted }}>😊 {c.mood} · 🔥 {c.urge}</div>
                </div>
                {rm && <div style={{ background:rm.bg, color:rm.text, fontSize:10, fontWeight:700, padding:"3px 8px", borderRadius:20, flexShrink:0, textTransform:"capitalize" }}>{c.risk}</div>}
              </Card>
            );
          })}

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginTop:8 }}>
            <button onClick={exportCSV} style={{ padding:"13px 10px", background:"transparent", border:`1.5px solid ${S.border}`, borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer" }}>
              📥 CSV Export
            </button>
            <button onClick={enableReminder} disabled={notifStatus==="denied"}
              style={{ padding:"13px 10px", background:notifStatus==="granted"?S.text:"transparent", color:notifStatus==="granted"?S.bg:S.text, border:`1.5px solid ${S.border}`, borderRadius:12, fontSize:13, fontWeight:700, cursor:"pointer" }}>
              {notifStatus==="granted" ? "🔔 Aktiv" : "🔔 Reminder"}
            </button>
          </div>

          {notifStatus==="denied" && <div style={{ fontSize:11, color:"#b71c1c", textAlign:"center", marginTop:8 }}>Benachrichtigungen blockiert. Bitte in den Browser-Einstellungen erlauben.</div>}
          {notifStatus==="granted" && <div style={{ fontSize:11, color:"#2e7d32", textAlign:"center", marginTop:8 }}>Tägliche Erinnerung aktiv.</div>}
        </>}
      </div>
    )}
    ...
    {tab === "denken" && (
      <div style={{ animation:"fadeUp 0.35s ease both" }}>
          ...
          {DENK_MODI.map(m => (
            <button key={m.id} onClick={() => { setDenkModus(m.id); setDenkResult(""); setDenkError(""); }}
              style={{
                ...
                border:`1.5px solid ${denkModus===m.id ? S.text : S.border}`,
                background: denkModus===m.id ? S.text : "transparent",
                color: denkModus===m.id ? S.bg : S.text,
        ...
        <Btn onClick={submitDenken} disabled={!denkInput.trim() || denkLoading}>
          {denkLoading ? <Spinner /> : `${DENK_MODI.find(m=>m.id===denkModus)?.icon} Analysieren`}
        </Btn>

        {denkResult && (
          <Card style={{ lineHeight:1.7, fontSize:14, marginTop:10, animation:"fadeUp 0.3s ease both" }}>
  ...
  {/* ── TAB BAR ── */}
  <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:S.card, borderTop:`1px solid ${S.border}`, display:"flex", paddingBottom:"env(safe-area-inset-bottom,0)" }}>
    {TABS.map(t => (
      <button key={t.id} onClick={() => setTab(t.id)}
        style={{ flex:1, background:"none", border:"none", cursor:"pointer", padding:"10px 0 8px", display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
        <span style={{ fontSize:20 }}>{t.icon}</span>
        <span style={{ fontSize:10, fontWeight:700, color:tab===t.id?S.text:S.muted }}>{t.label}</span>
        {tab === t.id && <div style={{ width:4, height:4, borderRadius:"50%", background:S.text }} />}
      </button>
    ))}
  </div>
</div>
